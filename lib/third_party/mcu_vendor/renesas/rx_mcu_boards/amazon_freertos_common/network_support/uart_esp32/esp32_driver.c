...
//static int32_t memdrv_open(uint32_t);
...
int32_t esp8266_ssl_connect(uint8_t socket_no, uint32_t ipaddr, uint16_t port)
{
	...
	strcpy((char *)buff,"AT+CIPSTART=");
	sprintf((char *)buff+strlen((char *)buff),"%d,\"SSL\",\"%d.%d.%d.%d\",%d\r\n",socket_no, (uint8_t)(ipaddr>>24),(uint8_t)(ipaddr>>16),(uint8_t)(ipaddr>>8),(uint8_t)(ipaddr),port);
	ret = esp8266_serial_send_with_recvtask(ESP8266_UART_COMMAND_PORT, "AT+CIPSSLCCONF=0,1,0,0\r\n", 3, 10000, ESP8266_RETURN_OK, ESP8266_SET_CIPSTART, socket_no);
	if( 0 != ret )
	{
		return -1;
	}
