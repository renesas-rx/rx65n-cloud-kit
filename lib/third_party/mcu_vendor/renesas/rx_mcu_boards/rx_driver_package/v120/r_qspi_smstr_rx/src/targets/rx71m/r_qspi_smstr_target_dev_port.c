...
* Version      : 1.12
* Device       : RX
* Abstract     : Source file dedicated to RX71M for QSPI single master driver
...
*              : 28.11.2014 1.07     Added R_QSPI_SMstr_1ms_Interval(), r_qspi_smstr_start_timer(),
*              :                     r_qspi_smstr_check_timer() and r_qspi_smstr_end_timer().
*              :                     Changed r_qspi_smstr_tx_dmacdtc_wait() and r_qspi_smstr_rx_dmacdtc_wait().
*              : 30.01.2015 1.08     Added RX71M.
*              : 30.09.2016 1.09     Changed Tool-Chain version.
*              :                     Changed pin name.
*              :                     Added nop() to wait for the write completion.
*              : 31.07.2017 1.10     Modified register protect processing.
*                                    Fixed to correspond to Renesas coding rule.
*              : 04.04.2019 1.12     Added support for GNUC and ICCRX.
*              :                     Fixed coding style. 
*******************************************************************************/
...
void r_qspi_smstr_dataio2_init(uint8_t channel)
{
    ...
    #if (('8' != R_QSPI_SMSTR_CFG_QSPI_QIO2_PORT) && ('0' != R_QSPI_SMSTR_CFG_QSPI_QIO2_BIT))
...
void r_qspi_smstr_dataio3_init(uint8_t channel)
{
    ...
    #if (('8' != R_QSPI_SMSTR_CFG_QSPI_QIO3_PORT) && ('1' != R_QSPI_SMSTR_CFG_QSPI_QIO3_BIT))
...
void r_qspi_smstr_clk_init(uint8_t channel)
{
    ...
    #if (('7' != R_QSPI_SMSTR_CFG_QSPI_QSPCLK_PORT) && ('7' != R_QSPI_SMSTR_CFG_QSPI_QSPCLK_BIT))
