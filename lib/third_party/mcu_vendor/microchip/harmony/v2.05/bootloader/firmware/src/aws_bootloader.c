...
// #define AWS_BOOT_DEBUG_MASK         (AWS_BOOT_DEBUG_BASIC | AWS_BOOT_DEBUG_VALIDATE | AWS_BOOT_DEBUG_LAUNCH | AWS_BOOT_DEBUG_LAUNCH_FLAGS | AWS_BOOT_DEBUG_COMMANDS)
#define AWS_BOOT_DEBUG_MASK         (AWS_BOOT_DEBUG_VALIDATE | AWS_BOOT_DEBUG_LAUNCH) 
...
static bool     app_img_flags_change;        // if changing of the image flags is enabled

static bool     app_flash_map;              // if mapping of the flash is enabled

static const uint8_t* app_exec_address = 0; // application execution address override
...
static AWS_BOOT_STATE AWS_State_ValidateImages(void)
{
    ...
    if(app_exec_address != 0)
    {   // forced address in debug... we could launch from a different bank so we enable the mapping 
        const AWS_BOOT_IMAGE_DCPT* exec_img = (const AWS_BOOT_IMAGE_DCPT*)KVA0_TO_KVA1(app_exec_address);
        // not really important, but needed for flash mapping
        app_img_to_exec = exec_img < app_img_up_ptr ? app_img_low_ptr : app_img_up_ptr;
        app_img_flags_change = false;
        return AWS_BOOT_STATE_START_EXEC;
    }

    // nothing...
#if (AWS_ENABLE_DEFAULT_START != 0)
        app_exec_address = (const uint8_t*)AWS_DEFAULT_FLASH_IMAGE_START;
        app_img_flags_change = false;
        app_flash_map = false;
        return AWS_BOOT_STATE_START_EXEC;
...
static AWS_BOOT_STATE AWS_State_ExecuteImage(void)
{
    ...
    {
#if ((AWS_BOOT_DEBUG_MASK & AWS_BOOT_DEBUG_LAUNCH) != 0)
        SYS_CONSOLE_PRINT("AWS Launch:  No Map performed. Running directly from address: 0x%08x?\r\n", app_exec_address);
#endif  //((AWS_BOOT_DEBUG_MASK & AWS_BOOT_DEBUG_LAUNCH) != 0)
    }


    const void* app_launch = app_exec_address == 0 ? (const void*)app_img_to_exec->entry_address : app_exec_address;
#if  ((AWS_BOOT_DEBUG_MASK & AWS_BOOT_DEBUG_COMMANDS) != 0)
...
static int BootCmdFlags(SYS_CMD_DEVICE_NODE* pCmdIO, int argc, char** argv)
{
    ...
    bool enableChange = app_img_flags_change;
    bool setRun = false;
    int run_flags = 0;
    ...
    if(!setRun)
    {
        ...
        app_img_flags_change = enableChange;
