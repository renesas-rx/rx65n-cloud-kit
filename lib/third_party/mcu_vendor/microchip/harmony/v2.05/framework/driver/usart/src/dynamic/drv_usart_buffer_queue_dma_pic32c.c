 ...
 * Include files.
 ************************************************************/
#include "../drv_usart_local_dma_pic32c.h"
    ...
    *bufferHandle = DRV_USART_BUFFER_HANDLE_INVALID;
    ...
    if(hDriver->interruptNestingCount == 0)
    {
        ...
        if(OSAL_MUTEX_Lock(&(hDriver->mutexDriverInstance), OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
        {
            ...
            {
                _DRV_USART_InterruptDisable(hDriver->interruptDMA);
            }
    ...
    else
    {
        ...
        if(SYS_DMA_CHANNEL_HANDLE_INVALID != hDriver->dmaChannelHandleWrite)
        {
            _DRV_USART_InterruptEnable(hDriver->interruptDMA);
        }
    ...
    if(hDriver->queueWrite == NULL)
    {
        ...
        if(SYS_DMA_CHANNEL_HANDLE_INVALID != hDriver->dmaChannelHandleWrite)
        {
            ...
            SYS_DMA_ChannelTransferAdd(hDriver->dmaChannelHandleWrite, bufferObj->buffer, bufferObj->size * 2,
                           (const void*)&hDriver->moduleId->US_THR, destSize,cellSize);
        }
        else
        {
    ...
    *bufferHandle = DRV_USART_BUFFER_HANDLE_INVALID;
    ...
    if(hDriver->queueWrite == NULL)
    {
        ...
        else
        {
            ...
            while(0 == hDriver->moduleId->US_CSR.TXEMPTY);
            /* Send first byte */
            hDriver->moduleId->US_THR.w |= US_THR_TXCHR( bufferObj->buffer[0]);

            bufferObj->nCurrentBytes ++;
           
            /* If the driver is configured for interrupt mode, then the following
             statement should enable the interrupt source for transmit to process 
             DRV_USART_trasksTransmit task. */
...
/* Function:
  ...
  Remarks:
    Not supported if instance of the driver uses DMA mode.
*/
...
size_t DRV_USART_BufferCompletedBytesGet(DRV_USART_BUFFER_HANDLE bufferHandle)
{
     ...
     * or DMA mode enabled then return DRV_USART_BUFFER_HANDLE_INVALID */
