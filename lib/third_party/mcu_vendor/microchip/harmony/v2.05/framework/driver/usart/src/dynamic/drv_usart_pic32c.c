...
void  DRV_USART_Deinitialize(SYS_MODULE_OBJ object)
{
    ...
    _DRV_USART_InterruptSourcesDisable(dObj->moduleId);
...
void DRV_USART_TasksTransmit(SYS_MODULE_OBJ object)
{
    ...
    /* Execute transmit task if data is transmitted */
...
void _DRV_USART_HardwareSetup(usart_registers_t *moduleId, DRV_USART_INIT * init)
{
     ...
     * Calling these functions have no effect in polled mode */
    _DRV_USART_ByteModelRXInterruptSourceEnable(moduleId);
...
void _DRV_USART_ByteTransmitTasks (SYS_MODULE_OBJ object)
{
    ...
    if (1 == hDriver->moduleId->US_IMR.TXEMPTY)
    {
...
void _DRV_USART_ByteReceiveTasks (SYS_MODULE_OBJ object)
{
    ...
    if ( 1 == hDriver->moduleId->US_IMR.RXRDY )
    {
...
void _DRV_USART_ByteErrorTasks (SYS_MODULE_OBJ object)
{
    ...
    if ( (1 == hDriver->moduleId->US_IMR.OVRE) || (1 == hDriver->moduleId->US_IMR.FRAME) ||(1 == hDriver->moduleId->US_IMR.PARE) )
    {
        /* Clear error condition */
        _DRV_USART_ErrorConditionClear(hDriver);
