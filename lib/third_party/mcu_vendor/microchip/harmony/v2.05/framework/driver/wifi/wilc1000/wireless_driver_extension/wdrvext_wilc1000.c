...
static bool wdrv_ext_InitComplete = false;
...
static int32_t _WDRV_EXT_Initialize(WDRV_HOOKS const *const hooks)
{
	...
	WDRV_SEM_INIT(&g_wdrvext_priv.scanDoneWait);
	...
	memcpy(&(s_wilc1000_intf.wdrvIndications), 
		&(hooks->wdrvIndications),
		sizeof(hooks->wdrvIndications));
...
static void _WDRV_EXT_Deinitialize(void)
{
	...
	WDRV_SEM_DEINIT(&g_wdrvext_priv.scanDoneWait);
...
uint32_t WDRV_EXT_CmdScanStart(void)
{
	...
	WDRV_SEM_TAKE(&g_wdrvext_priv.scanDoneWait, OSAL_WAIT_FOREVER);
...
{
    WDRV_DBG_INFORM_MESSAGE(("Scan in progress\r\n"));
    s_scanControl.scanListGetOnce = false;
    s_scanControl.scanInProgress = true;
}


bool WDRV_EXT_IsScanComplete(void)
{
    return WDRV_WILC1000_IsScanComplete();
}
...
void WDRV_WILC1000_Tasks2()
{
    if(wdrv_ext_InitComplete)
    {        
        if(!s_isInitComplete) 
         {      
            if (s_wilc1000_intf.ethMode || s_wilc1000_intf.fwOtaServerUrl)
                 wilc1000_task_init(&s_wilc1000_intf);
             else
                 wilc1000_task_init2(&s_wilc1000_intf);
        }
        else 
        {
             wilc1000_task(&s_wilc1000_intf);
        }
    }
