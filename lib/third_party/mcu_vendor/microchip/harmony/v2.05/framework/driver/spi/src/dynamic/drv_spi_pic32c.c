...
#include "driver/spi/src/drv_spi_local_pic32c.h"
...
{  
    ...
    /* Root driver initialization if the driver if first instance */
    if (sNumDriverInit == 0)
    {
    ...
    if (index >= DRV_SPI_INSTANCES_NUMBER)
    {
        ...
        /* If it's first driver reset driver initialization */
        if (sNumDriverInit == 0) 
        {
...
DRV_HANDLE DRV_SPI_Open ( const SYS_MODULE_INDEX index, const DRV_IO_INTENT intent )
{
    ...
    /* Initialize client object parameters */
                                              ...
                                              void * context, DRV_SPI_BUFFER_HANDLE * jobHandle)
{
    ...
    DRV_SPI_JOB_OBJECT * pJob = NULL;
     ...
     /* Check if Driver was opened for READ or not */
    if(!(pClient->intent & DRV_IO_INTENT_READ))
    {
        /* Driver is not opened in READ mode */
                                              ...
                                              void * context, DRV_SPI_BUFFER_HANDLE * jobHandle )
{
    ...
    {
        /* Driver is not opened in Write mode */
        SYS_ASSERT(false, "\r\nSPI Driver: Driver is not opened in WRITE mode.");
        return (DRV_SPI_BUFFER_HANDLE)DRV_SPI_BUFFER_HANDLE_INVALID;
    }
                                 ...
                                 const DRV_SPI_CLIENT_DATA * cfgData  )
{
    ...
    {
        /* just save the new baud rate value for this client, don't change 
         * it here. Actual change will happen in the task routine when queue 
         * request is serviced */
