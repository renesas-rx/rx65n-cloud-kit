...
/******************************************************************************
  Function:
    CLK_SOURCES_SYSTEM _SYS_CLK_ClockTypeMap( OSC_SYS_TYPE clk )
  Summary:
    Maps the OSC_SYS_TYPE to CLK_SOURCES_SYSTEM enum.

  Description:
    This function Maps the OSC_SYS_TYPE to CLK_SOURCES_SYSTEM enum.
...
CLK_SOURCES_SYSTEM _SYS_CLK_ClockTypeMap(OSC_SYS_TYPE clk)
{
    switch(clk)
    {
        ...
        case OSC_USBCLK_WITH_PLL:
            return SYS_CLK_SOURCE_UPLL;
        case OSC_LPRC:
...
uint32_t _SYS_CLK_PLLOutputDividerRead ( void )
{
    return PLIB_OSC_SysPLLOutputDivisorGet( OSC_PLIB_ID ) + 1;
}
