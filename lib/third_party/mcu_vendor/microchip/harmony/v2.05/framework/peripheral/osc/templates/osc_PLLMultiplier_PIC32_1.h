...
{
    if(( pll_multiplier < 22u) && ( pll_multiplier > 14u ))
    {
        OSCCONbits.PLLMULT = (pll_multiplier - 15u);
    }
    else if( pll_multiplier == 24u )
    {
        OSCCONbits.PLLMULT = (pll_multiplier - 17u);
    }
    else
    {
...
{
    uint8_t multiplier;
    OSC_SYSPLL_MULTIPLIER_TYPE returnValue = 0u;

    multiplier = OSCCONbits.PLLMULT;

    /* Factor to actual value conversion */
    if ( ( multiplier >= 0u ) && ( multiplier < 7u ) )
    {
        returnValue = (multiplier + 15u);
    }
    else if ( multiplier == 7u )
    {
        returnValue = (multiplier + 17u);
    }
