...
PLIB_TEMPLATE void USART_Disable_Default( USART_MODULE_ID index )
{
    ...
    usart->UxMODECLR = UxMODE_ON_MASK;
}
...
{
    usart_registers_t volatile * usart = ((usart_registers_t *)(index));
    usart->UxMODESET = UxMODE_ON_MASK;
}
...
{
    usart_registers_t volatile * usart = ((usart_registers_t *)(index));
    uint32_t brgValueLow = 0;
    uint32_t brgValueHigh = 0;

    /* Enable transmit and receive */
    usart->UxSTASET = (UxSTA_UTXEN_MASK | UxSTA_URXEN_MASK);

    /* Find the BRG Value */
    brgValueLow  = ( ((systemClock >> 4u) + (baud >> 1u)) / baud ) - 1u;
    brgValueHigh = ( ((systemClock >> 2u) + (baud >> 1u)) / baud ) - 1u;
    ...
    if ((brgValueHigh >= 0) && (brgValueHigh <= UINT16_MAX))
    {
        ...
        usart->UxBRG = brgValueHigh;
    }
    else
    {
        /* Clear BRGH */
        usart->UxMODECLR = UxMODE_BRGH_MASK;
        usart->UxBRG = brgValueLow;
    }
